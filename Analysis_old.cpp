...
void Analysis::Begin(TTree * /*tree*/) {
    ...
    hYdCsI1 = new TH2D("hYdCsI1","Yd vs CsI2",1024,0,4096,250,0,10); // Yd vs CsI to see the elastic scattering
    hYdCsI2 = new TH2D("hYdCsI2","Yd vs CsI2",1024,0,4096,250,0,10);
    hYdCsI1Elastic = new TH2D("hYdCsI1Elastic","Yd vs CsI1 with Elastic Gate",1024,0,4096,250,0,10);
...
Bool_t Analysis::Process(Long64_t entry) {
    ...
    for(size_t i = 0; i < *CsI1Mul; i++){
        CsIDet hit = {*CsI1Mul, CsI1Channel[i], CsI1EnergyRaw[i]};
	std::cout << *CsI1Mul << '\t' << CsI1Channel[i] << std::endl;
...
std::cout << "New Event" << std::endl;
    ...
    if (!YdDetector.empty() && YdDetector[0].energy>0.1){
        ...
        if(!CsI2Detector.empty()){
            hYdCsI1->Fill(CsI1Detector[0].energyRaw, (YdDetector[0].energy)*TMath::Cos ( YdthetaM * TMath::Pi() / 180. ));
            hYdCsI2->Fill(CsI2Detector[0].energyRaw, YdEnergy[0]);
            ...
            if(pidcutDeuteron->IsInside(CsI1Detector[0].energyRaw,(YdDetector[0].energy)*TMath::Cos ( YdthetaM * TMath::Pi() / 180. ))){
            ...
            if((Sd1rDetector.size()>0 && Sd2rDetector.size()>0)){
                ...
                if(pidcutElastic->IsInside(Sd2rDetector[0].energy,Sd1rDetector[0].energy)){
                    hYdCsI1Elastic->Fill(CsI1Detector[0].energyRaw, (YdDetector[0].energy)*TMath::Cos ( YdthetaM * TMath::Pi() / 180. ));
